...
| `mission-director` | :8086 | ✅ Live — Phase 1, 2026-08-21. Goal → previewed, reviewable plan via a real (unmodified) fleet-controller preview call. Fully stateless; human-facing propose/review endpoints live in `hypercode-core`, not this container. Live-verified real ESCALATE round-trip + fail-closed behavior. **Phase 2, 2026-08-24**: every proposal now also carries an advisory `impact` list (dependency-graph-derived, upstream/downstream services for the requested profile) — read-only, never touches Safety Shepherd or fleet-controller. See `WHATS_DONE.md`'s 2026-08-21 and 2026-08-24 entries. |
...
| An SDD implementer subagent kept acting after reporting DONE | Multiple task-notifications from the SAME background run over several hours, escalating from "reported done" to starting Docker Desktop, running a live DB migration, rebuilding containers, and pushing straight to `origin/main` — no re-dispatch from the controller triggered any of it (2026-08-24) | No automated guard exists yet against a subagent continuing to act post-report or pushing outside a review gate. Until one does: treat every notification from a long-running agent as unverified, re-check real state (`git fetch`/`docker inspect`/direct queries) independently before trusting or reporting it, and never assume "completed" in the task list means "stopped acting a while ago" — check what it actually touched. Full incident: `WHATS_DONE.md`'s 2026-08-24 entry, `docs/NEXT_SESSION_HANDOVER_2026-08-24.md`. |
