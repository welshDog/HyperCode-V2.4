...
| 2 | **Launch 25-agent fleet** — blocked: item #0's permanent fix (rename/retire/distinct-names decision), 1b, and `business-agent` all need resolving first, and item #0 currently means "don't even try" | 🔴 blocked, not ready |
...
| P2-2 | ✅ **4 of 5 fixed 2026-08-20 evening**, verified via `docker compose config` (not grep): `system-architect` :8008→:8010 (was colliding with live `healer-agent`), `hyper-split-agent` :8096→:8013 (was colliding with live `safety-shepherd`), `session-snapshot` :8097→:8017 (was colliding with live `evolve-relay`, `--profile pets`), `tips-tricks-writer` :8009→:8018 (was colliding with live `chroma`, found this pass — not in the original list). ❌ **`test-agent` vs `hyper-brain` (:8100) left open** — it's also one of the 14 same-name-merge cases in item #0 above, so it needs that decision first, not just a port move. Also found the same pass: the launch command's missing `--profile agents` flag (not a port issue — see item #0/#1 above, now documented as required). | `docker-compose.agents-full.yml` |
...
**Remaining open collision: only `test-agent` vs `hyper-brain` (:8100)** — left alone since it's entangled with item #0's same-name-merge decision, not a simple port move.
