...
| 2 | **Launch 25-agent fleet** — blocked: item #0's permanent fix (rename/retire/distinct-names decision) and `business-agent` still need resolving, and item #0 currently means "don't even try" | 🔴 blocked, not ready |
...
**All real port-vs-differently-named-service collisions in `agents-full.yml` are now fixed.** Only item #0 (the 14-name same-name-merge architecture decision) and `business-agent` (P2-1) remain open.
