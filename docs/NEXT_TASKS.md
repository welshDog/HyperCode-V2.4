...
| 2 | **Launch 25-agent fleet** — blocked on two independent things now: item #0's permanent fix (rename/retire/distinct-names decision, currently means "don't even try") AND item #9 (20 of 24 agents can't build or can't be reached on their host port — not fixed, only audited). `business-agent` is the one exception, fully fixed 2026-08-20. | 🔴 blocked, not ready |
...
| 9 | **✅ AUDIT COMPLETE, 2026-08-20 late evening — all 24 agents in `agents-full.yml` checked, not fixed.** Confirmed the item-#9 hypothesis at fleet scale: `agents-full.yml` uniformly maps `HOST:8080`, but each agent's own Dockerfile/base template bakes its own internal port independently, never reconciled against that. **Full breakdown below** — 4 fine, 17 port-mismatched, 3 can't even build. See `docs/AGENTS_FULL_PORT_AUDIT_2026-08-20.md` for the complete per-agent evidence table. | 🔴 confirmed fleet-wide, none fixed except business-agent |
| 9a | **✅ Fine (4):** `crew-orchestrator` (CMD hardcodes `--port 8080`), `hyper-architect` (Node, `ENV PORT=8080`), `test-agent`, `business-agent` (both fixed earlier tonight) — all genuinely listen on `8080`. | — |
| 9b | **🔴 Port-mismatched (17), same 3-way pattern as `system-architect`/`tips-tricks-writer`/`coderabbit-webhook`:** `agent-x` (8000), `coder-agent` (defaults 8000, also `FROM agent-base:latest` — that base image exists locally already, built from `agents/agent-base.Dockerfile`, so not currently broken but is an extra build-order dependency worth knowing about), `frontend-specialist` (8002), `backend-specialist` (8003), `database-architect` (8004), `qa-engineer` (8005), `devops-engineer` (8006), `security-engineer` (8007), `project-strategist` (8001), `goal-keeper` (8050, confirmed via `main.py`'s `os.getenv("AGENT_PORT", "8050")`), `hyper-split-agent` (8096), `session-snapshot` (8097), `throttle-agent` (8014), `super-hyper-broski-agent` (8015). Every one of these has its own Dockerfile healthcheck baked to its **old, pre-reconciliation host port** — a strong signal these Dockerfiles were written before `agents-full.yml` standardized on `HOST:8080`, and never updated. `AGENT_PORT` is never set via `agents-full.yml`'s own `environment:` blocks for any service (verified, zero matches) — nothing overrides these at compose time. | `docker compose build` succeeds for these, but the host-mapped port would route to a dead container port |
| 9c | **🔴 Can't even build (3), worse than a port mismatch — a build-context path bug, same class as the `hypercode-mcp-server` phantom fixed earlier tonight:** `brain-agent` — `context: ./agents/brain` doesn't exist as a directory at all. `hyper-observer`/`hyper-worker` — `agents-full.yml` expects `context: ./agents/hyper-agents, dockerfile: Dockerfile.observer`/`Dockerfile.worker` at the top of that directory; the real Dockerfiles exist one level deeper, misplaced at `agents/hyper-agents/observer/Dockerfile` and `agents/hyper-agents/worker/Dockerfile` (both agents' actual code confirms further port mismatches too, once the path is fixed: `AGENT_PORT=8092`/`8093` baked, still not `8080`). | `docker compose build` fails outright for these 3 |
