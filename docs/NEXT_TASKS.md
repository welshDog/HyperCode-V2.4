...
| 2 | **Launch 25-agent fleet** — item #9 (all 24 agents building + binding `8080` correctly) and item #0 (the same-name-merge decision) are both now fully closed. `docker compose --profile agents --profile hyper -f docker-compose.yml -f docker-compose.agents-full.yml up -d` is verified safe via `docker compose config` (zero collisions) but has **not been run** — Bro's call 2026-08-20 was files-only this session, launch is a separate step whenever ready. | 🟢 ready, not yet launched |
