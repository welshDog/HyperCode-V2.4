...
> ⚠️ **"Launch 25-agent fleet" below is NOT ready — down to a single blocker now.**
> Item #9 (every agent's container-internal port matching compose's uniform `HOST:8080`
> mapping, including the 3 that couldn't even build) is now **fully closed** as of
> 2026-08-20 late evening (commit `84fa5a2d`) — all 24 agents build and pass a live
> `/health` check. **The only thing left is item #0**: 14 of ~24 agent names in
> `agents-full.yml` are also defined in `docker-compose.agents.yml` with different build
> contexts/ports/profiles, and compose silently merges same-named services across `-f`
> files instead of erroring. Bro's call (2026-08-20) is to not compose the two files
> together until that's resolved for real — see item #0 below.
...
| 2 | **Launch 25-agent fleet** — item #9 (all 24 agents building + binding `8080` correctly) is now fully closed, including the 3 former item #9c agents. **Only item #0 remains**: the permanent fix for the 14-name same-name-merge decision (rename/retire/distinct-names, currently means "don't even try"). | 🔴 blocked on item #0 only |
