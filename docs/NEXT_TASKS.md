...
| 0b | **New, not fixed — found 2026-08-23 night while finishing 0a's live verification.** `frontend-specialist` has no `ANTHROPIC_API_KEY` in its compose environment at all (`docker exec frontend-specialist printenv ANTHROPIC_API_KEY` → empty) — its `/execute` still returns `200`, but the LLM call inside fails with a generic "Connection error" instead of falling back to Ollama the way `project-strategist`/`broski-coo` do. Not individually checked, but likely shared by `backend-specialist`, `database-architect`, `qa-engineer`, `devops-engineer`, `security-engineer`, `system-architect` — same specialist-squad tier, probably the same compose-block gap. Needs: audit all 7 specialist blocks in `docker-compose.agents.yml` for `ANTHROPIC_API_KEY`/fallback wiring. | 🟢 found, not fixed |
