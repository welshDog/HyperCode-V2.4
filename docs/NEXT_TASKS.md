...
| 2a | **New, found during launch, not fixed:** `throttle-agent` logs `"Error while fetching server API version: ('Connection aborted.', FileNotFoundError(2, 'No such file or directory'))"` on every health check — it tries to talk to the Docker socket (presumably for its resource-throttling logic) but `agents-full.yml`'s definition has no `/var/run/docker.sock` volume mount at all. Also logs `[Throttle] MemStream unreachable: All connection attempts failed` repeatedly. Neither is fatal — the container reports itself `"status":"degraded"` but its HTTP healthcheck still returns 200, so Docker shows it healthy — but the throttling feature itself likely isn't working. Pre-existing, not caused by tonight's changes (only the networks: list was touched here, unrelated to the socket). | `docker-compose.agents-full.yml`'s `throttle-agent` block |
