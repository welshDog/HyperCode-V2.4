...
| Phase 10E rollout (4 of 15 identities ACTIVE 2026-06-12) | LIVE: crew-orchestrator (`task`), nemoclaw-agent (`health`), broski-pets-bridge (`pets`), healer-agent (`ops` — BOTH heal pathways: legacy /heal+webhook AND MAPE-K execute) publish to `POST /api/v1/events` as THEMSELVES via `X-Agent-Key`. 15 keys minted (`scripts/mint_agent_keys.py <name>` mints single agents; ⚠️ no-arg run ROTATES the fleet). REMAINING: 11 specialist/util agents get the pattern when they gain core-bound calls. Orchestrator→agent inbound stays shared `${API_KEY}` by design | 🟢 LOW |
