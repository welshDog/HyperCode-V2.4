...
| Endpoint | Purpose |
|---|---|
| `POST /v1/capabilities/mint` | Body: proposal + `plan_hash` + `mode` + requested `action`/`target`. Validates schema/allowlist (reuse `fleet-controller`'s `plan_validator`), calls Shepherd `/evaluate` for that `plan_hash`, applies the transition table (§7). Mints a capability **only** if the table says so AND kill-switch clear AND (approval satisfied where required) AND (lease valid, **`LIVE` mode only** — a `DRY_RUN` mint needs no lease). Otherwise returns the structured verdict with `capability: null` and a machine reason. Writes `verdict.issued` + (`capability.minted` \| `mint.refused`) ledger rows. |
| `POST /v1/capabilities/verify` | Public-key verification is offline-doable; this endpoint adds the stateful checks (`jti` unused, not revoked, kill-switch, lease) for callers that want them centralized. Default (`burn` omitted/`false`) is an idempotent read — repeatable, no replay protection. The **executor** is the only caller that should ever pass `burn: true`, and only immediately before performing the action — that call registers the `jti` in the replay store, making every subsequent verify for it fail. Any caller that treats a non-burning verify as authorization to act reintroduces replay risk. |
...
| `POST /v1/kill` / `POST /v1/unkill` | `kill`: authenticated with an **operator token** (a dedicated `OPERATOR_KEY`/`OPERATOR_KEY_FILE` secret — the shared dashboard-session/API secret is deliberately *not* an accepted fallback, closed by the Phase 2 follow-up fix that dropped `main.py`'s hardcoded default pointing at it) **or** triggered by the sentinel file `/governance/KILL` appearing on a watched volume (file present ⇒ killed, even with the API and Redis down). `unkill`: API + operator token + a mandatory `reason` — deleting the file alone never un-kills. Both write ledger rows. |
