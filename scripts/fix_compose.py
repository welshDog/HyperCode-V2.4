...
def main():
    ...
    backup_file = compose_file + ".backup2"
    ...
    # Find the line index where networks starts
    networks_idx = None
    ...
    if networks_idx is None:
        print("Error: Could not find networks section")
        sys.exit(1)
    ...
    # Insert the service block before the networks line
    new_lines = lines[:networks_idx] + service_block + lines[networks_idx:]

    with open(compose_file, 'w', encoding='utf-8') as f:
        f.writelines(new_lines)

    print(f"Inserted meta-research-architect service before networks line at index {networks_idx}")
